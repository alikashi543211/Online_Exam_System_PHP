...
<?php
 ...
 $user_id=$_SESSION['user_id'];
  ...
  $query_x="SELECT q_paper_id, q_paper_type_id, course_id, user_id FROM q_paper
 WHERE q_paper_type_id='$q_paper_type_id' && course_id='$course_id' && user_id='$user_id'";
 $result_x=mysqli_query($con,$query_x);

while($row_x=mysqli_fetch_array($result_x))
{
	$q_paper_id=$row_x['q_paper_id'];
}

  $_SESSION['q_paper_id']=$q_paper_id;
...
<?php
                       ...
                       //******************************************************************************//
	// ----------------------------------------- SUBJECTIVE FORMATE INSERTION IN MIX PAPER ---------------------------------
	...
	// ----------------------------------------- OBJECTIVE FORMATE INSERTION IN MIX PAPER ---------------------------------
	...
	else if(isset($_SESSION['mcq_objective']) && isset($_SESSION['fb_objective']))
	{
		$mcq="MCQs";
		$fb="Fill in Blanks";
		$tf="-----";
	}
	else if(isset($_SESSION['mcq_objective']) && isset($_SESSION['tf_objective']))
	{
		$mcq="MCQs";
		$fb="-----";
		$tf="True False";
	}
	else if(isset($_SESSION['fb_objective']) && isset($_SESSION['tf_objective']))
	{
		$mcq="MCQs";
		$fb="Fill in Blanks";
		$tf="-----";
	}
	else if(isset($_SESSION['mcq_objective']))
	{
		$mcq="MCQs";
		$fb="-----";
		$tf="-----";
...
if(isset($_POST['mix_create_paper']))
{
	...
	$query_f="INSERT INTO q_p_formate(formate, total_marks, mcq, fb, tf, sq, lq, q_paper_id, status)
	                               VALUES('$formate', '$total_marks', '$mcq', '$fb', '$tf', '$sq', '$lq', '$q_paper_id', 'Active')";
...
// if paper is mix paper then query will execute one time only	
   ...
   //_______________________________________Subjective Formate Insertion__________________________________________
	...
	if(isset($_SESSION['short_questions']) && isset($_SESSION['long_questions']))
	{
		...
		$status="Active";
		$query_e="INSERT INTO sq (t_marks_sq,quan_sq,q_p_formate_id,status)
		                         VALUES('$t_marks_sq','$quan_sq','$q_p_formate_id','$status')";
		...
		$status="Active";
		$query_p="INSERT INTO lq (t_marks_lq, quan_lq,q_p_formate_id,status)
		                         VALUES('$t_marks_lq','$quan_lq','$q_p_formate_id','$status')";
...
<body style="background-color:lightgrey;padding:none;margin:none;">
<form action="../index.php" method="post">
<table id="table1" style="background-color:lightgrey;padding:none;">
<tr>
...
?>
<?php echo "<h1 style='text-align:left;color:navy;'>Question Paper:- <center>".($_SESSION['course_name']." ( ".$_SESSION['q_paper_type']." )</h1></center>"); ?>
</td>
<td style="height:70px; width:250px;text-align:center;"><button style="border-radius:4px;border:none;height:50px;width:100px;background-color:navy;color:white;cursor:pointer;"  name="sign_out" />Sign Out </button>
</td>
</tr>
<tr>
</tr>
...
<tr>
<td><input style="border-radius:4px;border:none;height:50px;width:150px;background-color:navy;color:white;cursor:pointer;" type="submit" name="objective" value="Objective (Paper)" /></td>
<td><input style="border-radius:4px;border:none;height:50px;width:150px;background-color:navy;color:white;cursor:pointer;" type="submit" name="subjective" value="Subjective (Paper)" /></td>
<td><input style="border-radius:4px;border:none;height:50px;width:150px;background-color:navy;color:white;cursor:pointer;" type="submit" name="mix" value="Mix (Paper)" /></td>
</tr>
</table>
</form>
...
{                                                                
 ?>
	<form method="post">
    <table align="center" style="color:navy;">
	<tr>
	<td>Total Marks :</td>
	...
	<td colspan="2" align="right" >
				 ...
				 if (isset($check_total))         { echo "<span style='color:green;'>".$check_total."</span>"; }
           else  if (isset($error_total))         { echo "<span style='color:red;'>".$error_total."</span>"; } 
...
if(isset($mcq_choices))
{
 $_SESSION['mcq_choices'] = $mcq_choices;;
}
...
<?php
	if(isset($_POST['quan_sq1']) && isset($_POST['quan_sq2']) && isset($_POST['quan_sq']) && isset($_POST['t_marks_sq'])) 
		{ 
	    $_SESSION['quan_sq1']=$_POST['quan_sq1'];
		$_SESSION['quan_sq2']=$_POST['quan_sq2'];
		$_SESSION['t_marks_sq']=$_POST['t_marks_sq'];
        $quan_sq1=$_SESSION['quan_sq1'];
		$quan_sq2=$_SESSION['quan_sq2'];
		$check_total=$quan_sq1+$quan_sq2;
		$quan_sq=$_POST['quan_sq'];
		if($check_total==$quan_sq)
		{
		$_SESSION['quan_sq']=$quan_sq;	
		}
		else
		{
			$error_msg1="in-correct";
		}
		//$quan_sq=$_SESSION['quan_sq'];
		$t_marks_sq=$_SESSION['t_marks_sq'];
...
<?php
	if(isset($_POST['quan_lq1']) && isset($_POST['quan_lq2']) && isset($_POST['quan_lq']) && isset($_POST['t_marks_lq'])) 
		{ 
	    $_SESSION['quan_lq1']=$_POST['quan_lq1'];
		
		$_SESSION['quan_lq2']=$_POST['quan_lq2'];
		$_SESSION['t_marks_lq']=$_POST['t_marks_lq'];
        $quan_lq1=$_SESSION['quan_lq1'];
		$quan_lq2=$_SESSION['quan_lq2'];
		$check_total=$quan_lq1+$quan_lq2;
		$quan_lq=$_POST['quan_lq'];
		if($quan_lq==$check_total)
		{
			$_SESSION['quan_lq']=$quan_lq;
		}
		else
		{
			$error_msg2="in-correct";
		}
		//$quan_lq=$_SESSION['quan_lq'];
		$t_marks_lq=$_SESSION['t_marks_lq'];
...
if(isset($_SESSION['short_questions']) && isset($_SESSION['long_questions']))
		{
		    ...
		    if($total==$_SESSION['total_marks'] && !isset($error_msg1) && !isset($error_msg2))
			{
				$check="correct.";
			}
            else 
...
else if(isset($_POST['mix_send_formate']))
{
	$total2=$_SESSION['t_marks_sq']+$_SESSION['t_marks_lq'];
		    if($total2==$_POST['total_marks2'] && !isset($error_msg1) && !isset($error_msg2))
			{
				$check="correct.";
			}
            else 
		...
		else if(isset($_SESSION['short_questions']))
		{
			...
			if($total==$_SESSION['total_marks'] && !isset($error_msg1))
			{
				$check="correct.";
			}
			else 
		...
		$total2=$_SESSION['t_marks_sq']+0;
			if($total2==$_POST['total_marks2'] && !isset($error_msg1))
			{
				$check="correct.";
			}
			else 
		...
		else if(isset($_SESSION['long_questions']))
		{
			...
			if($total==$_SESSION['total_marks'] && !isset($error_msg2))
			{
				$check="correct.";
			}
			else 
		 ...
		 else  if(isset($_POST['mix_send_formate']))
		 {
			...
			if($total2==$_POST['total_marks2'] && !isset($error_msg2))
			{
				$check="correct.";
			}
			else 
...
<!---------------------------------------Disapearance Of Options Of CheckBoxes---------------------------------------------->
...
else if(isset($_POST['choice1']) && isset($_POST['choice2']))
{
	 $choice3_unset="Choice 3 is not selected";
}
else if(isset($_POST['choice2']) && isset($_POST['choice3']))
{
	$choice1_unset="Choice 1 Has Unset";
}
else if(isset($_POST['choice1']) && isset($_POST['choice3']))
{
	$choice2_unset="Choice 2 has Unset";
}
else if(isset($_POST['choice1']))
{
	$choice2_unset="Choice 2 Has Unset";
	$choice3_unset="Choice 3 Has Unset";
}
...
<!---------------------------------------Disapearance Of Options ( MIX ) Of CheckBoxes------------------------------------>

<?php

if(isset($_POST['m1_choice1']) && isset($_POST['m1_choice2']) && isset($_POST['m1_choice3']))
{
	 
}
else if(isset($_POST['m1_choice1']) && isset($_POST['m1_choice2']))
{
	 $m1_choice3_unset="Choice 3 is not selected";
}
else if(isset($_POST['m1_choice2']) && isset($_POST['m1_choice3']))
{
	$m1_choice1_unset="Choice 1 Has Unset";
}
else if(isset($_POST['m1_choice1']) && isset($_POST['m1_choice3']))
{
	$m1_choice2_unset="Choice 2 has Unset";
}
else if(isset($_POST['m1_choice1']))
{
	$m1_choice2_unset="Choice 2 Has Unset";
	$m1_choice3_unset="Choice 3 Has Unset";
}
else if(isset($_POST['m1_choice2']))
{
	$m1_choice1_unset="Choice 1 Has Unset";
	$m1_choice3_unset="Choice 3 Has Unset";
}
else if(isset($_POST['m1_choice3']))
{
	$m1_choice1_unset="Choice 1 Has Unset";
	$m1_choice2_unset="Choice 2 Has Unset";
}
...
if(isset($_POST['m2_choice1']) && isset($_POST['m2_choice2']))
{
	 
}

else if(isset($_POST['m2_choice1']))
{
	$m2_choice2_unset="Choice 2 Has Unset";
	$m2_choice3_unset="Choice 3 Has Unset";
}
else if(isset($_POST['m2_choice2']))
{
	$m2_choice1_unset="Choice 1 Has Unset";
	$m2_choice3_unset="Choice 3 Has Unset";
}
...
<?php
  ...
  ?>
	...
	?>
	<?php
// _____________________________________________Check Total Marks_____________________________________________________

	if(isset($_POST['submission']) || isset($_POST['mix_send_formate']))
	{
		$total_marks1=$_POST['total_marks1'];
		$total_marks2=$_POST['total_marks2'];
		$set_total=$_POST['total_marks'];
		$calculated_total=$total_marks1+$total_marks2;
		if($calculated_total==$set_total)
		{
			$true="Marks Division is Correct.";
		}
		else
		{
			$false="No.";
...
// _____________________________________________/Check Total Marks_____________________________________________________	
	?>
	
    <table  align="center" style="color:navy;">
	<tr>
	<td colspan="2" align="center"><h1 style="color:navy;">Objective</h1></td>
	</tr>
...
<!---------------------------------------------------TOTAL MARKS INTERFACE------------------------------------------------>
	...
	<td colspan="2" align="right" >
				 ...
				 <?php 
				                    //check total marks and display check after send
				 if (isset($false))         { echo "<span style='color:red;'>".$false."</span>"; }					
			else if (isset($check4))         { echo "<span style='color:green;'>".$check4."</span>"; }
           else  if (isset($error4))         { echo "<span style='color:red;'>".$error4."</span>"; } 
...
<!--------------------------------------------------MCQS INTERFACE----------------------------------------------------------->
   ...
   <?php if(!isset($choice1_unset) && !isset($m1_choice1_unset))
   {  
	   ?>	
	<tr> 
	     <td colspan="2">
...
<?php  
	if(isset($_POST['choice1'])  || (isset($_POST['m1_choice1']) && isset($true)))
	{
	?>
	<tr>
	<td colspan="2">
				 ...
				 <tr>
				 <td>MCQ Options :</td>
				 <td><input type="text" name="mcq_choices" placeholder="Enter Options" 
				      value="<?php if(isset($mcq_choices)) { echo $mcq_choices; } ?>"           /></td>
	             </tr>
				 <tr>
				 ...
				 <?php if (isset($check1))   { echo "<span style='color:green;'>".$check1."</span>"; }
                else   if (isset($error1))   { echo "<span style='color:red;'>".$error1."</span>"; }   ?>
				                 </td>
				 </tr>
	       </table>
...
<!------------------------------------------------FILL IN BLANKS INTERFACE--------------------------------------------------->
	...
	<?php if(!isset($choice2_unset) && !isset($m1_choice2_unset))
   {  
	   ?>
	<tr>
	<td colspan="2">
	                   <input type="checkbox" <?php if(!isset($_POST['mix']) && !isset($chice2_unset) && !isset($_POST['submission']) && !isset($_POST['mix_send_formate']))  { ?> name="choice2" <?php } else { ?>
...
<!------------------------------------------/FILL_IN_BLANKS FORMATE INTERFACE------------------------------------------------>	
	<?php  
 if(isset($_POST['choice2'])  || (isset($_POST['m1_choice2']) && isset($true)))
	...
	<td colspan="2">                                        <!--Formate Of Fill_In_Blanks Interface-->
	        ...
	        <table style="color:maroon;">
	            ...
	            <tr>
				 ...
				 <?php if (isset($check2))   { echo "<span style='color:green;'>".$check2."</span>"; }
                 else  if (isset($error2))   { echo "<span style='color:red;'>".$error2."</span>"; }   ?>
				                 </td>
				 </tr>
	       </table>
...
<!--------------------------------------------------TRUE_FALSE INTERFACE----------------------------------------------------->
	<?php if(!isset($choice3_unset) && !isset($m1_choice3_unset))
   {  
	   ?>
	<tr>
	<td colspan="2">
	                 <input type="checkbox" <?php if(!isset($_POST['mix']) && !isset($chice3_unset) && !isset($_POST['submission']) && !isset($_POST['mix_send_formate']))  { ?> name="choice3" <?php } else { ?>
	name="m1_choice3" <?php } ?> value="True False Statements" 
	   ...
	   ?>
	<?php  
	if(isset($_POST['choice3'])  || (isset($_POST['m1_choice3']) && isset($true)))
	{
	?>
	<tr>
	<td colspan="2">
...
<!--------------------------------------------------TRUE_FALSE FORMATE INTERFACE--------------------------------------------->													   
				 ...
				 <?php if (isset($check3))   { echo "<span style='color:green;'>".$check3."</span>"; }
                 else  if (isset($error3))   { echo "<span style='color:red;'>".$error3."</span>"; }   ?>
				               </td>
				 </tr>
				      </table>
<!--------------------------------------------------/TRUE_FALSE FORMATE INTERFACE--------------------------------------------->			 
...
<?php if(!isset($_POST['mix']) && !isset($_POST['submission']) && !isset($_POST['mix_send_formate']))           //when mix set then submit button not displayed
	...
	<tr>
	     <td colspan="2" align="right">
		                        ...
		                        <input style="border-radius:4px;border:none;height:30px;width:100px;background-color:navy;color:white;cursor:pointer;" type="submit" name="<?php echo $btn1_name;   ?>"    
	     ...
	     <td style="float:right;" colspan="2" align="left">
		         <form method="post">         
		                        <br><!--<input style="float:left;border-radius:4px;border:none;height:30px;width:150px;background-color:navy;color:white;cursor:pointer;" type="submit" name="change_formate1"    
...
<?php
    ...
    <table align="center" style="color:navy;">
	<tr>
	<td colspan="2" align="center"><h1 style="color:navy;">Subjective</h1></td>
	</tr>
...
<!------------------------------------------------------TOTAL MARKS INTERFACE------------------------------------------------>
	...
	<td colspan="2" align="right" >
				 ...
				 <?php 
				  ...
				  if (isset($false))         { echo "<span style='color:red;'>".$false."</span>"; }	//Message of Error Total Check in Mix				
				else  if (isset($check))         { echo "<span style='color:green;'>".$check."</span>"; }
...
<!----------------------------------------------SHORT QUESTIONS INTERFACE---------------------------------------------------->
	<?php if(!isset($choice1_unset) && !isset($m2_choice1_unset)) { ?>	  
	<tr> 
	     <td colspan="2">
	                        <input type="checkbox" <?php if(!isset($_POST['mix']) && !isset($_POST['submission']) && !isset($_POST['mix_send_formate']))  { ?> name="choice1" <?php } else { ?>
...
<?php  
	if(isset($_POST['choice1']) || (isset($_POST['m2_choice1']) && isset($true)))
	{
	?>
    ...
    value="<?php if(isset($t_marks_sq)) { echo $t_marks_sq; } ?>" required	/></td>
	</tr>
	
	<tr style="color:maroon;">
	<td>Total Questions:</td>	
	<td colspan="2"><input type="text" name="quan_sq" placeholder="Total Quantity"
    value="<?php if(isset($quan_sq)) { echo $quan_sq; } ?>"	required /></td>
	
	</tr>
	
	<tr style="color:maroon;">
	<td>Attempt:</td>	
	<td colspan="2"><input type="text" name="quan_sq1" placeholder="Attempt Quantity"
    value="<?php if(isset($quan_sq1)) { echo $quan_sq1; } ?>" required	/></td>
	
	</tr>
	<tr style="color:maroon;">
	<td>Extra:</td>
	<td><input type="text" name="quan_sq2" placeholder="Extra Quantity"
    value="<?php if(isset($quan_sq2)) { echo $quan_sq2; } ?>" required	/></td>
	...
	<td colspan="2" align="right" >
				 ...
				 <?php 
				                    //check total marks and display check after send			
				  if (isset($check))         { echo "<span style='color:green;'>".$check."</span>"; }
           else  if (isset($error))         { echo "<span style='color:red;'>".$error."</span>"; } 
...
<!------------------------------------------------LONG QUESTIONS INTERFACE--------------------------------------------------->
	<?php if(!isset($choice2_unset) && !isset($m2_choice2_unset)) { ?>
	<tr>
	<td colspan="2">
...
<!-------------------------------------------LONG QUESTIONS FORMATE INTERFACE------------------------------------------------>	
	<?php  
 if(isset($_POST['choice2']) || (isset($_POST['m2_choice2']) && isset($true)))
    ...
    value="<?php if(isset($t_marks_lq)) { echo $t_marks_lq; } ?>" required	/></td>
	
	</tr>
	
	<tr style="color:maroon;">
	<td>Total Questions:</td>	
	<td colspan="2"><input type="text" name="quan_lq" placeholder="Total Quantity"
    value="<?php if(isset($quan_lq)) { echo $quan_lq; } ?>" required	/></td>
	
	</tr>
	
	<tr style="color:maroon;">
	<td>Attempt:</td>
	<td><input type="text" name="quan_lq1" placeholder="Attempt Quantity"
    value="<?php if(isset($quan_lq1)) { echo $quan_lq1; } ?>" required	/></td>
	
	</tr>
	<tr style="color:maroon;">
	<td>Extra:</td>
	<td><input type="text" name="quan_lq2" placeholder="Extra Quantity"
    value="<?php if(isset($quan_lq2)) { echo $quan_lq2; } ?>" required	/></td>
...
<!-------------------------------------------------------BUTTON INTERFACE---------------------------------------------------->		
	...
	<tr>
	     <td colspan="2" align="right">
		                        <input style="border-radius:4px;border:none;height:30px;width:100px;background-color:navy;color:white;cursor:pointer;" type="submit" name="<?php echo $btn2_name;   ?>"    
	     ...
	     <td style="float:left;" colspan="2" align="left">
		                  <form method="post">
		                        <br><!--<input style="float:left;border-radius:4px;border:none;height:30px;width:150px;background-color:navy;color:white;cursor:pointer;" type="submit" name="change_formate2"    
	                                  value="Change Formate"     />-->                  <!--Button 2 -->
...
<!-------------------------------------------------------/BUTTON INTERFACE--------------------------------------------------->
	...
	?>
<?php
// __________________________________________Mix Submit Section__________________________________________________________

if (isset($_POST['submission']) && isset($true))
		{
...
if(isset($_POST['mix']) || isset($_POST['submission']) || isset($_POST['mix_send_formate']))
{
	...
	<table align="center">
	<tr>
	     <td colspan="2" style="width:250px;" align="right">
		                        <input style="border-radius:4px;border:none;height:30px;width:100px;background-color:navy;color:white;cursor:pointer;" type="submit" name="<?php echo $b_name1; ?>"    
	     ...
	     <td style="float:left;" colspan="2" align="left">
		                  <form method="post">
		                        <br><!--<input style="float:left;border-radius:4px;border:none;height:30px;width:150px;background-color:navy;color:white;cursor:pointer;" type="submit" name="mix_change_formate"    
	                                  value="Change Formate"     />-->                  <!--Button 2 -->
	...
	<?php
// __________________________________________ /Mix Submit Section __________________________________________________________	
}
if(isset($_POST['submission']))
{
	
	//print_r ("<span style='color:white;'".$_POST."</span>");
}
