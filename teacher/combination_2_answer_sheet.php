...
if(isset($_SESSION['short_questions']) && isset($_SESSION['tf_ready']))
{
	$btn_name="goto";
	$btn_value="Next";
}
else if(isset($_SESSION['long_questions']) && isset($_SESSION['tf_ready']))
 {
	 $btn_name="goto";
     $btn_value="Next";	
    }
 ...
 if(isset($_GET['mcq_ans']))
{
    ...
    $_SESSION['mcq_ready']="MCQs (Options)";
	unset($_SESSION['tf_ready']);
	unset($_SESSION['fb_ready']);
...
{
	$_SESSION['tf_ready']="True False (Options)";
	unset($_SESSION['fb_ready']);
	unset($_SESSION['mcq_ready']);
}
 if(isset($_GET['fb_ans']))
{
   $_SESSION['fb_ready']="Fill In Blanks (Answers)";
	unset($_SESSION['tf_ready']);
	unset($_SESSION['mcq_ready']);
}
if(isset($_GET['show_ques']))
{
	unset($_SESSION['fb_ready']);
	unset($_SESSION['mcq_ready']);
	header ('Location:combination_2.php');
...
if((isset($_SESSION['edit_answers']) || isset($_SESSION['set_btn2']) || isset($_SESSION['return_back']))
	&& !isset($_SESSION['refresh'])  && !isset($_SESSION['edit_update']))
{
	...
	<tr>
	<td><input style="border:none;cursor:pointer;width:200px;height:50px;background-color:navy;color:white;border-radius:4px;"
...
<?php if (isset($_SESSION['edit_update'])) { ?> disabled <?php } ?>	type="submit" name="tf_ans" value="Show True False (Answers)" /></td>
	<td><input style="border:none;cursor:pointer;width:200px;height:50px;background-color:navy;color:white;border-radius:4px;"
<?php if (isset($_SESSION['edit_update'])) { ?> disabled <?php } ?>	type="submit" name="show_ques" value="Edit Questions" /></td>
...
//_____________________________________________Edit Question______________________________________2_________________-->
 /*(isset($_REQUEST['edit_answer_id']) && !isset($_SESSION['edit_recycle_bin2']) ||
...
<table border="1" align="center">
 ...
 <td>Question_ID</td>
  <td>Question</td>
  <?php if(isset($_SESSION['mcq_ready']) || isset($_SESSION['tf_ready'])) { ?>
  <td>Option</td>
  <?php } ?>
  <?php if(isset($_SESSION['mcq_ready']) || isset($_SESSION['tf_ready'])) { ?>
  ...
  <?php } ?>
  <td>Status</td>
  <?php if(isset($_SESSION['done']) && !isset($_SESSION['tf_ready'])) { ?>
...
?>
 <tr style="text-align:center;">
  ...
  <?php if(isset($_SESSION['done']) && !isset($_SESSION['tf_ready'])) { ?>
  <td><a href="combination_2_answer_sheet.php?edit_answer_id=<?php echo $row_b['ans_id']; ?>"> Edit</a></td>
  <td><a href="combination_2_answer_sheet.php?delete_answer_id=<?php echo $row_b['ans_id']; ?>"> Delete</a></td>
  <?php } ?>
</tr>
<?php	
}	
...
}// OVER ALL WHEN WE COME FOR EDITING...
