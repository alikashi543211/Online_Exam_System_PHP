...
{                                                                 // insertion into database
	...
	$ques_type  =   $_POST['q_type'];                          
	$status     =   $_POST['status'];                             	      
    $query_a = "INSERT INTO 
	          q_paper_type(ques_type,status)     
	          VALUES('$ques_type','$status')"; //insertion query
...
<?php
       if(isset($_REQUEST['edit_id']) && !isset($_POST['Update']))
	   {
		   ...
		   $query_e      =    "SELECT * FROM q_paper_type where q_paper_type_id='$edit_id' ";    //select row from database on particular id query
...
<!---------------------------------------------------Section 5--------------------------------------------------------------->
	...
	$up_q_type     =  $_POST['q_type'];
	$status        =  $_POST['status'];         
                                                                                            // update query
	   $query_f      =    "UPDATE q_paper_type SET ques_type='$up_q_type',status='$status'
	                      WHERE q_paper_type_id='$update_id' ";
...
<!---------------------------------------------------Section 6--------------------------------------------------------------->										<!--Deletion Of Student Data-->
  <?php
   if(isset($_REQUEST['delt_id']))
   {
	   ...
	   $query_g      =   "DELETE FROM q_paper_type WHERE q_paper_type_id='$delete_id' ";                     //delete query
...
?></td>
	...
	<br>
<h1 align="center" style="color:white;">Question-Paper-Type Details</h1>
  <br>
...
?>
  ...
  <tr>
                                 <td id="td3">Question-Type:</td>
     <td>
	 
	 <input type                 =              "text"
	        name                 =              "q_type"  
	        placeholder          =              "Question Paper Type"
	        value                =              "<?php if(isset($edit_q_type))  echo $edit_q_type;  ?>"
	        required                             />
			                                             </td>
...
<!---------------------------------------------------Section 3--------------------------------------------------------------->
	...
	<?php
   $query_b="SELECT * FROM q_paper_type";               // query for display all data
 ...
 while($row_b=mysqli_fetch_array($result_b))
  {
	 ?>
  <tr align="center">
             <td align="center">    <?php echo $row_b["q_paper_type_id"];       ?>        </td>
             <td align="left">      <?php echo $row_b["ques_type"];             ?>        </td>
             <td align="left">      <?php echo $row_b["status"];                ?>        </td>
             <td>
			 <a style="color:white" href="question_paper_type.php?edit_id=<?php echo $row_b["q_paper_type_id"]; ?> ">Edit</a>
			                                                                      </td>
             <td>
			 <a style="color:white" href="question_paper_type.php?delt_id=<?php echo $row_b["q_paper_type_id"]; ?> ">Delete</a>
			                                                                      </td>
  </tr>
	<?php
}
