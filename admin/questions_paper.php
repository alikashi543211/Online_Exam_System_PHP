...
?>
	...
	<br>
<h1 align="center" style="color:white;" >Question Paper Information</h1>
  <br>
...
?>
  ...
  <tr>
                                 <td id="td3">Question Paper ID:</td>
     <td>
	 ...
	 <input type                 =              "text"
	        ...
	        required                             />
                                   ...
                                   <td id="td3">Question Paper Type:</td>
                                   <td>
...
<!---------------------------------------------------Section 3--------------------------------------------------------------->
   ...
   */
   /*
   $query_b="SELECT p.status,p.q_paper_type_id,p.course_id,p.q_paper_id,p.user_id,
   t.q_paper_type_id,t.ques_type,c.course_id,c.course_name
   u.user_id,u.user_name,u.u_type_id,k.u_type_id,k.u_type FROM q_paper p, q_paper_type t,
   course c, user u, user_type k
   WHERE (p.q_paper_type_id=t.q_paper_type_id) && (p.course_id=c.course_id && p.user_id=u.user_id)
   && (u.u_type_id=k.u_type_id && 
   k.u_type='teacher')";
   */
   $query_b="SELECT u.user_id, u.user_name, u.u_type_id,
   k.u_type_id, k.u_type, c.course_id,
   c.course_name, t.q_paper_type_id, t.ques_type, p.status, p.q_paper_type_id, 
   p.course_id,p.q_paper_id, p.user_id  FROM user u, user_type k,
   course c, q_paper_type t, q_paper p WHERE ((p.user_id=u.user_id) && (u.u_type_id=k.u_type_id)
   && (k.u_type='teacher') && (p.course_id=c.course_id) && (p.q_paper_type_id=t.q_paper_type_id))";
   
   
   /*$query_b="SELECT c.course_id,c.course_name,t.q_paper_type_id,
   t.ques_type,p.q_paper_id,p.course_id,p.q_paper_type_id,p.course_id,p.user_name,p.status FROM  
   course c, q_paper_type t, q_paper p WHERE 
   (p.q_paper_type_id=t.q_paper_type_id && p.course_id=c.course_id)";
   */
   $result_b=mysqli_query($con,$query_b);
   if(!$result_b)
   {
   ...
   //$query_b="SELECT * FROM q_paper,q_paper_type,user, course,user_type WHERE user_type.u_type='student' && q_paper_type.q_paper_type_id='Mid Term'";
   //$result_b=mysqli_query($con,$query_b);
?>
...
<?php
 while($row_b=mysqli_fetch_array($result_b))
  ...
  <tr align="center">
			 ...
			 <td align="center">      <?php echo $row_b["course_name"];      ?>        </td>
