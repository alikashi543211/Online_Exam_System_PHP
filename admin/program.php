...
{                                                                 // insertion into database
	...
	$p_name  =   $_POST['p_name'];                          
	$status   =   $_POST['status'];                           
	                                                     // values from user-interface to variables
	      
    $query_a = "INSERT INTO 
	          program(program_name,status)     
	          VALUES('$p_name','$status')"; //insertion query
...
<?php
       if(isset($_REQUEST['edit_id']) && !isset($_POST['Update']))
	   {
		   ...
		   $query_e      =    "SELECT * FROM program where program_id=$edit_id ";    //select row from database on particular id query
		   ...
		   $edit_p_name       =   $row_e['program_name'];
...
<!----------------------------------------------- ---Section 5--------------------------------------------------------------->
    ...
    $up_p_name       =  $_POST['p_name'];                                      // edit_id from url
	$up_status       =  $_POST['status'];         
                                                                                            // update query
	   $query_f      =    "UPDATE program SET program_name='$up_p_name', 
	                      status='$up_status' WHERE program_id='$update_id'";
...
<!----------------------------------------------- ---Section 6--------------------------------------------------------------->										<!--Deletion Of Student Data-->
   ...
   if(isset($_REQUEST['delt_id']))
   {
	   ...
	   $query_g      =   "DELETE FROM program WHERE program_id='$delete_id'";                     //delete query
	   $result_g     =   mysqli_query($con,$query_g);                                       // query run
	   if($result_g)
	   {
	   $check_delete =  "Successfully deleted from database";                             //check result of delete true case
	   }
	   else
	   {
...
// end of edit check
?>
<h1 align="center" style="color:white;" >Program Details</h1>
		                                         
     <form method="post">
     <table align="center">
   
  <tr>
                                 <td id="td3">Enter Program name :</td>
     <td>
	 
	 <input type                 =              "text"
	        name                 =              "p_name"
	        placeholder          =              "Program Name"
	        value                =              "<?php if(isset($edit_p_name))  echo $edit_p_name;  ?>"
	        required                             />
			                                             </td>
	                                     
	 
                                   <tr>
						   <td style="color:white" >Enter Status :</td>
...
<!---------------------------------------------------Section 3--------------------------------------------------------------->
	...
	<?php
   $query_b="SELECT * FROM program";              // query for display all data
 ...
 while($row_b=mysqli_fetch_array($result_b))
  {
	 ?>
  <tr align="center">
             <td align="center">    <?php echo $row_b["program_id"];       ?>        </td>
             <td align="left">      <?php echo $row_b["program_name"];     ?>        </td>
             <td align="center">    <?php echo $row_b["status"];        ?>        </td>
             <td>
			 <a style="color:white" href="program.php?edit_id=<?php echo $row_b["program_id"]; ?>">Edit</a>
			                                                                      </td>
             <td>
			 <a style="color:white" href="program.php?delt_id=<?php echo $row_b["program_id"]; ?>">Delete</a>
			                                                                      </td>
  </tr>
	<?php
}
