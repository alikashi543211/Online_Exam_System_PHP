...
<html>
   <head>
    ...
    <title>
Admin/Teacher-Course-Allocation
    </title>
  </head>
<?php
$btn="Allocate";
   ...
   foreach($_POST['allocate'] as $key=>$course_id )
	   {
		...
		$status=$_POST['status'];
        $query_a="INSERT INTO teach_course_alloc(user_id,course_id,status)
		                                VALUES('$user_id','$course_id','$status')     ";
        $result_a=mysqli_query($con,$query_a);										
	   }
	   if(isset($result_a)) // check insertion
		  {
			  $check_insert="Courses inserted successfully into Database.";  // true case              
		  }
   }
   else
	   {
	  ...
	  if(isset($_REQUEST['edit_id']) && !isset($_POST['Update']))
	   {
		   ...
		   $query_b="SELECT * FROM teach_course_alloc WHERE teach_c_al_id='$edit_id' ";
		...
		if(isset($_POST['Update']))
		{
			...
			if($count==1)
			{
             ...
             $up_status=$_POST['status'];		 
                                                                                            // update query
	        $query_f      =    "update teach_course_alloc
	                                SET 
	                      user_id   = '$up_user_id',
                          course_id = '$up_course_id',						  
	                      status    = '$up_status'
                          where                     teach_c_al_id='$update_id' ";
...
<!----------------------------------------------- ---Section 6--------------------------------------------------------------->										<!--Deletion Of Student Data-->
   ...
   if(isset($_REQUEST['deallocate_id']))
   {
	   ...
	   $query_g      =   "DELETE FROM teach_course_alloc WHERE teach_c_al_id='$deallocate_id'";                     //delete query
...
?>
	...
	<br>
<h1 align="center" style="color:white;" >Teacher Course Allocation</h1>
  <br>
...
?>
    ...
    $query_z="SELECT u.user_id,u.user_name,u.u_type_id,t.u_type_id, t.u_type FROM user u,user_type t where 
	         u.u_type_id=t.u_type_id && t.u_type='teacher' ";
...
<!---------------------------------------------------Section 3--------------------------------------------------------------->
	...
	<?php
   $query_b="SELECT a.teach_c_al_id,a.user_id,a.course_id,a.status,u.user_id,u.user_name,c.course_id,c.course_name
   From teach_course_alloc a,user u,course c where (u.user_id=a.user_id && a.course_id=c.course_id) ";               // query for display all data
   $result_b=mysqli_query($con,$query_b);
?>
 <table align="center" border="5" class="pclr">
  <tr align="center">
    <td>Teacher_Course_Allocation_ID</td>
    <td>Teacher_Name</td>
    <td>Course_Name</td>
    <td>Status</td>
 ...
 while($row_b=mysqli_fetch_array($result_b))
  {
	 ?>
  <tr align="center">
             <td align="center">    <?php echo $row_b["teach_c_al_id"];       ?>        </td>
             ...
             <td>
			 <a style="color:white" href="teach_course_alloc.php?edit_id=<?php echo $row_b["teach_c_al_id"]; ?>">Edit</a>
			                                                                      </td>
<td>
			 <a style="color:white" href="teach_course_alloc.php?deallocate_id=<?php echo $row_b["teach_c_al_id"]; ?>">Deallocate</a>
			                                                                      </td>
