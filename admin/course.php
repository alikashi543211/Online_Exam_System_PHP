...
{                                                                 // insertion into database
	...
	$program_id   =   $_POST['program_id'];                        // values from user-interface to variables   
	$status       =   $_POST['status'];                            
	      
    $query_a = "INSERT INTO 
	          course(course_name,program_id,status)     
	          VALUES('$course_name','$program_id','$status')"; //insertion query
...
<?php
       if(isset($_REQUEST['edit_id']) && !isset($_POST['Update']) && !isset($_POST['Add']))
	   {
		   $btn          =    "Update";
		   $edit_id      =    $_REQUEST['edit_id'];                            // edit_id from url
		   $query_e      =    "SELECT * FROM course where course_id=$edit_id ";    //select row from database on particular id query
		   ...
		   $edit_course_name     =   $row_e['course_name'];
		   $edit_program_id      =   $row_e['program_id'];
...
<!----------------------------------------------- ---Section 5--------------------------------------------------------------->
	...
	$status      =  $_POST['status'];         
                                                                                            // update query
	   $query_f      =    "UPDATE course SET course_name='$course_name', 
...
<!----------------------------------------------- ---Section 6--------------------------------------------------------------->										<!--Deletion Of Student Data-->
   ...
   if(isset($_REQUEST['delt_id']))
   {
	   ...
	   $query_g      =   "DELETE FROM course WHERE course_id='$delete_id'";                     //delete query
...
?>
  ...
  <tr>
	 ...
	 <input type                 =              "text"
	        ...
	        value                =              "<?php if(isset($edit_course_name))  echo $edit_course_name;  ?>"
	        required                             />
			                                             </td>
...
<tr>											
	...
	while($row_d=mysqli_fetch_array($result_d))             //fetch program_id and program_name from program table row by row
	{
		...
		<?php if(isset($edit_program_id) && ($edit_program_id==$row_d["program_id"]))
			{ ?> selected <?php } ?> > 
...
<!---------------------------------------------------Section 3--------------------------------------------------------------->
	...
	<?php
   $query_b="SELECT c.course_id,c.course_name,c.program_id,c.status,p.program_id,p.program_name FROM course c,program p where c.program_id=p.program_id";               // query for display all data
 ...
 while($row_b=mysqli_fetch_array($result_b))
  {
	 ?>
  <tr align="center">
             <td align="center">    <?php echo $row_b["course_id"];       ?>        </td>
             <td align="left">      <?php echo $row_b["course_name"];     ?>        </td>
             <td align="left">      <?php echo $row_b["program_name"];      ?>        </td>
             <td align="center">    <?php echo $row_b["status"];    ?>        </td>
             <td>
			 <a style="color:white" href="course.php?edit_id=<?php echo $row_b["course_id"]; ?>">Edit</a>
			                                                                      </td>
             <td>
			 <a style="color:white" href="course.php?delt_id=<?php echo $row_b["course_id"]; ?>">Delete</a>
			                                                                      </td>
  </tr>
	<?php
}
