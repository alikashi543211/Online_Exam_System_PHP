<head>
<a href="generate_leave_pdf.php" style="color:white;" >Generate Leave PDF</a>
 ...
 if(isset($_POST['sign_in']))
	{
	   ...
	   $query="SELECT * FROM user,user_type WHERE user.u_type_id=user_type.u_type_id";
		...
		while($row=mysqli_fetch_array($result))
		{
			...
			header("refresh:1; url=".$_SESSION['user_type']."/index.php");
			}			
...
<tr>
   <td class="email" ><input type="email" name="email_id" placeholder="Email Address" required></td>
   <td class="email" ><input type="password" name="password" placeholder="Password" required></td>
   ...
   {
   ?>
   <option value="<?php echo $row_a['u_type_id']; ?>" ><?php echo $row_a['u_type']; ?></option>
   <?php
   }
   ?>
   </select></td>
